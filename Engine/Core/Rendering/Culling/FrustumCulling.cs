...
{        
    ...
    public static class FrustumCulling
    {
        ...
        /// <returns></returns>
        public static bool Culling(AABB aabb, GameObject camera, GameObject obj, Matrix4x4 MVP, Matrix4x4 objectInvTransform)
        {
            //아래 두 함수는 해도 안 해도 컬링 여부에는 결과를 미치지 않습니다.
            //그러나, 미리 여기서 간단한 것들을 계산하고 넘어가면, 뒤쪽의 무거운 연산을 줄일 수 있습니다.
            if (Culling_IsObjectPositionViewed(obj.WorldPosition, MVP))
                return true;
            if (Culling_IsCameraInsideAABB(camera.WorldPosition, aabb, objectInvTransform))
                return true;
            ...
            for (int i = 0; i < 8; i++)
            {
                var corner = transformedCorners[i] = TransformMatrixCaculator.TransformH(corners[i], MVP);
                float w = corner.w;
        ...
        /// </summary>
        static bool Culling_IsCameraInsideAABB(Vector3 cameraPosition, AABB aabb, Matrix4x4 objectInvTransform)
        {
